				...
				<?php
					if(isset($liste_vehicules)) {
						...
						for($i=0;$i<count($liste_vehicules);$i++) {
							if($liste_vehicules[$i]->getVeh_type()->getCategorie()->getCat_libelle() == "Voiture") {
								...
								$res .= "<div id=\"veh_modele\"><p>".$liste_vehicules[$i]->getVeh_modele()->getVehmmod_libelle()."</p></div>";
								...
								$res .= "<li><img class=\"icone\" src=\"/PowerRenters/icones/doors.svg\">".$liste_vehicules[$i]->getVeh_porte()->getVeh_porte_libelle()." portes</li>";
								$res .= "</ul>";
